...
### Added
- Experimental foundation for running Cloudflare (workerd) SSR natively, with no Node, Miniflare, or Vite plugin. `oj_server::workerd` locates the `workerd` binary, generates its Cap'n Proto text config (a worker with a socket, a module-fallback address, and var/service bindings), and answers workerd's module-fallback requests with TS/JSX-stripped ESM served from oj's compiler. An end-to-end test renders a TypeScript route inside real workerd. Not yet wired into `oj dev`.
