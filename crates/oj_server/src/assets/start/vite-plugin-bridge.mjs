...
// The SSR loader and the client bundler are separate processes whose user-
// plugin buildStart runs now overlap (the runner spawns concurrently with the
// bundle); plugins that compile to shared on-disk state must not interleave.
// Cross-process single-flight: mkdir is the atomic acquire, a dead holder's
// lock is stolen so a crashed process can't wedge the other one.
