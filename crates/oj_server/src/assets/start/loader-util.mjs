...
// JSONL pack framing, format 2: every line is 8 hex digits of payload byte
// length + 16 hex digits of sha256(payload) + the JSON payload + "\n"; line 1
// is a header record { fmt, epoch }.
...
// Walks a pack file's records and returns the next append offset (the file
// size after any self-healing), or null when the file is absent, from another
// epoch, or from another format (a format mismatch deletes the file). A
// framing failure truncates the file to the last good offset; a hash failure
// (when verifyHashes) or an onRecord(rec, buf) === false skips that record.
