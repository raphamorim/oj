...
let specPending = [];
let specDirty = false;
// Writes are chained: an unordered append racing a dirty whole-file rewrite
// interleaves mid-file and tears the pack.
...
if (EPOCH) {
  // Index [payload offset, payload length, payload hash] per record; hashes
  // are checked on pread (or here, under OJ_CACHE_VERIFY=full) so init never
  // has to materialize the sources.
  const bytes = scanPack(LOAD_PACK_FILE, "load-pack", EPOCH, VERIFY_FULL, (rec, buf) => {
    // Payloads are written as {"k":"<64 hex>","c":...}; anything else is skipped.
