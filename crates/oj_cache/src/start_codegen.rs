...
impl StartCodegenStore {
    ...
    pub fn restore(
        ...
        // Restores write into the app tree, so the stored artifacts are
        // always re-hashed against the manifest recorded at persist time,
        // whatever the global verify mode.
...
/// Recorded size + blake3 of each artifact in an entry, written at persist
/// time and required at restore time: an entry without a readable manifest
