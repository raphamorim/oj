...
/// Self-verifying whole-file artifact for stores whose entry name is a
/// lookup key rather than a content hash: the payload's blake3 (hex) on
/// the first line, the payload after it. Written atomically.
...
/// Counterpart of [`write_self_verified`]: always re-hashes (these
/// artifacts are small), regardless of [`VerifyMode`].
