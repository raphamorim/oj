...
/// Cache for the vite.config extract subprocess output, keyed by the config
/// file plus its import graph (as reported by the extract itself), the
/// lockfiles/.env files, and the env vars a config commonly reads.
...
impl ConfigExtractStore {
    ...
    pub fn lookup(&self, config: &Path, command: &str, mode: &str) -> Option<CachedExtract> {
        ...
        // Entries are small: always re-hash, whatever the global verify mode.
...
/// Non-file inputs that can shape the extracted values: dependency lockfiles
/// (node_modules content), package.json, .env files, and the env vars app
/// configs commonly branch on. Anything here changing must be a cache miss.
